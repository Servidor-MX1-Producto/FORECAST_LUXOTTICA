...
q001CalcReq <- q001IdSemana %>% 
  ...
  mutate(REQUERIMIENTO_SS = NECESIDAD_SS - lag(NECESIDAD_SS, default = 0)) %>% #Utilizamos la funcion lag para hacer calculo entre rows anteriores, restando la necesidad conforma va avanzando las semanas
...
q001Requerimiento <- q001IdSemana %>% #Tomamos dataframe base para realizar los cruces
  ...
  left_join(q001CalcReq[,c("ID_ELPTS", "REQUERIMIENTO", "REQUERIMIENTO_SS")], by = "ID_ELPTS") %>% #Cruce con el requerimiento previamente calculado
  mutate_at(c("REQUERIMIENTO", "REQUERIMIENTO_SS"), ~replace(., is.na(.), 0)) %>% 
  mutate(REQUERIMIENTO_SS = ifelse(REQUERIMIENTO_SS < 0, 0, REQUERIMIENTO_SS)) %>% 
